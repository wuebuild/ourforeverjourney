...
import { Invitation } from "@/shared/types/api";
import { getMyInvitations } from "@/features/invitation/services/server";
...
async function getUserIdFromSession(): Promise<string | null> {
  ...
  const jar = cookies();
  const userId = (await jar).get("token")?.value || null; // set this at login
  return userId;
}
...
export default async function MyInvitationPage() {
  ...
  console.log("here invitations", invitations)
        ...
        {invitations.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {invitations.map((inv: Invitation) => {
              ...
              const dt = `Invitation Date: ${inv.date || '-'},${inv.time || '-'}`;
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <div className="text-xs text-gray-500">
                        Created {formatDate(inv.createdDate)}
                      </div>
                    </div>
