...
import { WeddingTemplateData } from "@/types/template";
...
export default function FPRomantic ({data} : {data:WeddingTemplateData}) {
...
function InvitationBody({data} : {data: WeddingTemplateData}) {
  ...
  return (
    <div className="relative bg-white text-gray-800">
      ...
      <section className="pt-8 pb-8 text-center">
        <h2 className="text-2xl md:text-3xl font-semibold">Save the Date</h2>
        <div className="mt-4 flex flex-col md:flex-row justify-center gap-4 md:gap-6 text-lg md:text-xl">
          {/* <div><strong>{diff.ds}</strong> Days</div>
          <div><strong>{diff.hs}</strong> Hours</div>
          <div><strong>{diff.ms}</strong> Minutes</div>
          <div><strong>{diff.ss}</strong> Seconds</div> */}
...
function CountdownSection({ diff }: { diff: diffTime}) {
  ...
  return (
    <section className="p-8 text-center bg-gradient-to-b from-pink-50 to-white">
      <h2 className="text-3xl md:text-4xl font-heading font-bold text-pink-700">
        Save the Date 💌
      </h2>
      ...
      <div className="mt-6 flex justify-center gap-4 md:gap-8">
        {[
          ...
          <div
            key={i}
            className="flex flex-col items-center bg-white shadow-lg rounded-xl w-20 h-20 md:w-24 md:h-24 justify-center border border-pink-200"
          >
...
function MusicPlayerFAB() {
  ...
  return (
    <>
      ...
      <button
        onClick={togglePlay}
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-pink-600 shadow-lg flex items-center justify-center hover:bg-pink-700 transition"
      >
        {isPlaying ? (
